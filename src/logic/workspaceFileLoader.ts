...
import { XP_DOCUMENTATION_FILES,
         ...
         XP_ICON_DEFINITIONS,
         LICENSE_TIER_DEFINITIONS } from "./constants";
...
export const loadLicenseTiers = async () => {
    const descriptor = (await getFileDescriptors(LICENSE_TIER_DEFINITIONS))[0];

    const file = await loadFile(descriptor);
    const tiers = JSON.parse(file);
    return tiers;
}
